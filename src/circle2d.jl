...
draw!(img::AbstractArray{T, 2}, circle::CirclePointRadius, color::T; opacity::AbstractFloat = 1.0) where {T<:Colorant} = draw!(img, Ellipse(circle), color, opacity=opacity)
...
function draw!(img::AbstractArray{T, 2}, circle::CircleThreePoints, color::T; opacity::AbstractFloat = 1.0) where T<:Colorant
    ind = axes(img)
    ...
    draw!(img, CirclePointRadius(Point(round(Int, R[1]), round(Int, R[2])), ρ), color, opacity=opacity)
end
