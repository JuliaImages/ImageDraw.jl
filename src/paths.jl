...
draw!(img::AbstractArray{T, 2}, path::Path, thickness::Int) where T<:Colorant =
	draw!(img, path, oneunit(T), thickness)
...
draw!(img::AbstractArray{T, 2}, polygon::Polygon, thickness::Int) where T<:Colorant =
	draw!(img, path, oneunit(T), thickness)
...
draw!(img::AbstractArray{T, 2}, rp::RegularPolygon, thickness::Int) where T<:Colorant =
	draw!(img, rp, oneunit(T), thickness)
