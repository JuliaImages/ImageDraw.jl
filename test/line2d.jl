...
facts("Lines2D") do
	...
	context("Bresenham") do
		...
		expected = copy(img)
		expected[diagind(expected)] = one(Gray{U8})
		res = bresenham(img, 1, 1, 10, 10, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[diagind(expected)[1:5]] = one(Gray{U8})
		res = bresenham(img, 1, 1, 5, 5, one(Gray{U8}))
		@fact expected == res --> true

		expected = copy(img)
		expected[10, :] = one(Gray{U8})
		res = bresenham(img, 10, 1, 10, 10, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[5, :] = one(Gray{U8})
		res = bresenham(img, 5, 1, 5, 10, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[5, 1:5] = one(Gray{U8})
		res = bresenham(img, 5, 1, 5, 5, one(Gray{U8}))
		@fact expected == res --> true

		expected = copy(img)
		expected[:, 10] = one(Gray{U8})
		res = bresenham(img, 1, 10, 10, 10, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[:, 5] = one(Gray{U8})
		res = bresenham(img, 1, 5, 10, 5, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[1:5, 5] = one(Gray{U8})
		res = bresenham(img, 1, 5, 5, 5, one(Gray{U8}))
		@fact expected == res --> true

		expected = copy(img)
		expected[10:9:91] = one(Gray{U8})
		res = bresenham(img, 1, 10, 10, 1, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[10:9:55] = one(Gray{U8})
		res = bresenham(img, 10, 1, 5, 6, one(Gray{U8}))
		@fact expected == res --> true

		img2 = zeros(RGB, 10, 10)
		expected = copy(img2)
		expected[diagind(expected)] = RGB{U8}(0.2, 0.3, 0.4)
		res = bresenham(img2, 1, 1, 10, 10, RGB{U8}(0.2, 0.3, 0.4))
		@fact expected == res --> true
	end
	...
	context("Xiaolin-Wu") do
		...
		res = xiaolin_wu(img, 1, 1, 10, 10, one(Gray{U8}))
		@fact expected == res --> true
		expected = copy(img)
		expected[diagind(expected)[1:5]] = one(Gray{U8})
		res = bresenham(img, 1, 1, 5, 5, one(Gray{U8}))
