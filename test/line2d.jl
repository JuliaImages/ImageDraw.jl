@testset "Lines2D" begin
	...
	@testset "LineSegment" begin
		...
		expected = copy(img)
		expected[diagind(expected)] .= oneunit(Gray{N0f8})
		res = @inferred draw(img, LineSegment(1,1,10,10))
		@test expected == res
		res = @inferred draw(img, LineSegment(1,1,10,10), bresenham)
		@test expected == res
		res = @inferred draw(img, LineSegment(1,1,10,10), oneunit(Gray), bresenham)
		@test expected == res
		res = @inferred draw(img, LineSegment(1,1,10,10), oneunit(Gray))
		@test expected == res
		...
		@test expected == res
		res = @inferred draw(img, LineSegment(CartesianIndex(1, 1), CartesianIndex(10, 10)), oneunit(Gray), bresenham)
		@test expected == res
		res = @inferred draw(img, LineSegment(CartesianIndex(1, 1), CartesianIndex(10, 10)), oneunit(Gray))
		@test expected == res
	...
	@testset "Bresenham" begin
		...
		expected = copy(img)
		expected[diagind(expected)] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 1, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[diagind(expected)[1:5]] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 1, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res

		expected = copy(img)
		expected[10, :] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 10, 1, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[5, :] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 5, 1, 5, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[5, 1:5] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 5, 1, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res

		expected = copy(img)
		expected[:, 10] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 10, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[:, 5] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 5, 10, 5, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[1:5, 5] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 5, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res

		expected = copy(img)
		expected[10:9:91] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 1, 10, 10, 1, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[10:9:55] .= oneunit(Gray{N0f8})
		res = @inferred bresenham(copy(img), 10, 1, 5, 6, oneunit(Gray{N0f8}))
		@test expected == res
	...
	@testset "Xiaolin-Wu" begin
		...
		res = @inferred xiaolin_wu(copy(img), 1, 1, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[diagind(expected)[1:5]] .= oneunit(Gray{N0f8})
		expected[1, 1] = Gray{N0f8}(0.5)
		expected[5, 5] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 1, 1, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res
		...
		expected[10, 10] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 10, 1, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[5, :] .= oneunit(Gray{N0f8})
		expected[5, 1] = Gray{N0f8}(0.5)
		expected[5, 10] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 5, 1, 5, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[5, 1:5] .= oneunit(Gray{N0f8})
		expected[5, 1] = Gray{N0f8}(0.5)
		expected[5, 5] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 5, 1, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res
		...
		expected[10, 10] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 1, 10, 10, 10, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[:, 5] .= oneunit(Gray{N0f8})
		expected[1, 5] = Gray{N0f8}(0.5)
		expected[10, 5] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 1, 5, 10, 5, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[1:5, 5] .= oneunit(Gray{N0f8})
		expected[1, 5] = Gray{N0f8}(0.5)
		expected[5, 5] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 1, 5, 5, 5, oneunit(Gray{N0f8}))
		@test expected == res
		...
		expected[10, 1] = Gray{N0f8}(0.5)
		expected[1, 10] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 1, 10, 10, 1, oneunit(Gray{N0f8}))
		@test expected == res
		expected = copy(img)
		expected[10:9:55] .= oneunit(Gray{N0f8})
		expected[10, 1] = Gray{N0f8}(0.5)
		expected[5, 6] = Gray{N0f8}(0.5)
		res = @inferred xiaolin_wu(copy(img), 10, 1, 5, 6, oneunit(Gray{N0f8}))
		@test expected == res
	end
	...
	@testset "LineTwoPoints" begin
		...
		@test expected == res
		res = @inferred draw(img, LineTwoPoints(1,1,10,10), oneunit(Gray), bresenham)
		@test expected == res
		res = @inferred draw(img, LineTwoPoints(1,1,10,10), oneunit(Gray))
		@test expected == res
		...
		@test expected == res
		res = @inferred draw(img, LineTwoPoints(CartesianIndex(1, 1), CartesianIndex(10, 10)), oneunit(Gray), bresenham)
		@test expected == res
		res = @inferred draw(img, LineTwoPoints(CartesianIndex(1, 1), CartesianIndex(10, 10)), oneunit(Gray))
		@test expected == res
		expected[diagind(expected)] .= Gray{N0f8}(0.5)
